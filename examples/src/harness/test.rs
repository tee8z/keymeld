...
pub async fn retry_bitcoin_rpc<T, F>(operation_name: &str, mut operation: F) -> Result<T>
where
...
pub struct KeyMeldE2ETest {
    ...
    pub taproot_tweak: TaprootTweak, // Configurable taproot tweak for keygen sessions
    // SDK clients for the new SDK-based implementation
    pub sdk_coordinator_client: Option<KeyMeldClient>,
    pub sdk_participant_clients: Vec<KeyMeldClient>,
}
...
impl KeyMeldE2ETest {
    pub async fn new(
        ...
        // Create SDK coordinator client
        let sdk_coordinator_credentials =
        ...
        Ok(Self {
            ...
            rpc_batcher,
            taproot_tweak: TaprootTweak::None, // Default: return raw untweaked aggregate key, client applies tweaks as needed
            sdk_coordinator_client: Some(sdk_coordinator_client),
            sdk_participant_clients: Vec::new(),
        })
    }

    /// Set a custom taproot tweak for keygen sessions
    /// Use `TaprootTweak::None` for compatibility with external MuSig2 implementations
    /// that don't apply tweaks (e.g., dlctix)
    pub fn with_taproot_tweak(mut self, tweak: TaprootTweak) -> Self {
    ...
    pub async fn load_participants(&mut self) -> Result<()> {
        ...
        for i in 1..self.config.num_signers as usize {
            ...
            // Create SDK client for this participant
            ...
            self.participant_user_ids.push(user_id);
    ...
    pub async fn fund_aggregate_key_address(
        ...
        let key_bytes = hex::decode(aggregate_key_hex)
            .map_err(|e| anyhow!("Failed to decode aggregate key hex: {}", e))?;
        let pubkey =
    ...
    /// Fund a specific TxOut (e.g., dlctix funding output) and return the outpoint.
    /// This is used when the funding address is computed externally (e.g., by dlctix)
    /// rather than derived from keymeld's aggregate key.
    pub async fn fund_dlctix_output(
        &mut self,
        ...
        let script_pubkey = bitcoin::ScriptBuf::from_bytes(script_pubkey_bytes.to_vec());
        ...
        let required_amount = funding_output.value.to_sat();
        let required_btc = required_amount as f64 / 100_000_000.0;

        // Use batcher if enabled, otherwise direct RPC
        let funding_txid = if let Some(ref batcher) = self.rpc_batcher {
            let txid_str = batcher
                .send_to_address(&address.to_string(), required_btc)
                .await?;
            bitcoin::Txid::from_str(&txid_str)
                .map_err(|e| anyhow!("Invalid txid from batcher: {}", e))?
        } else {
            retry_bitcoin_rpc("send_to_address (dlctix)", || {
                self.rpc_client_with_wallet.send_to_address(
                    &address,
                    Amount::from_sat(required_amount),
                    None,
        ...
        // Find the correct output index for our address
        let funding_tx_info = retry_bitcoin_rpc("get_transaction", || {
            self.rpc_client_with_wallet
                .get_transaction(&funding_txid, None)
        })
        ...
        let funding_tx = funding_tx_info.transaction()?;
        let target_script_pubkey = address.script_pubkey();
        ...
        let vout = found_vout.ok_or_else(|| {
            anyhow!(
                "DLC funding address {} not found in funding transaction outputs",
                address
            )
        })?;

        // Convert to dlctix OutPoint
        let txid_bytes: [u8; 32] = funding_txid.to_byte_array();
        let dlctix_txid = dlctix::bitcoin::Txid::from_byte_array(txid_bytes);
        let outpoint = dlctix::bitcoin::OutPoint::new(dlctix_txid, vout);
        Ok(outpoint)
    }
    ...
    pub async fn create_signing_session(
        ...
        // Encrypt the taproot tweak (use session-level tweak, defaulting to None)
    ...
    // =========================================================================
    // SDK-based methods - New high-level API using the KeyMeld SDK
    // =========================================================================

    /// Get the SDK coordinator client
    pub fn sdk_coordinator(&self) -> Result<&KeyMeldClient> {
        self.sdk_coordinator_client
            .as_ref()
            .ok_or_else(|| anyhow!("SDK coordinator client not initialized"))
    }

    /// Get an SDK participant client by index
    pub fn sdk_participant(&self, index: usize) -> Result<&KeyMeldClient> {
        self.sdk_participant_clients
            .get(index)
            .ok_or_else(|| anyhow!("SDK participant client {} not found", index))
    }

    /// Create a keygen session using the SDK
    ///
    ...
    ///
    /// This is a convenience method that handles the entire keygen process:
    /// 1. Creates the keygen session
    /// 2. Registers the coordinator
    ...
    ///
    /// This is a convenience method that handles the entire signing process:
    /// 1. Creates the signing session
    /// 2. Approves for coordinator
    /// 3. Approves for required participants
    /// 4. Waits for completion
    ///
    /// Returns the signature bytes.
    ///
    /// Note: This requires a keygen session that was created with `run_keygen_sdk`.
    /// It uses the session secret stored during keygen to restore session credentials.
    pub async fn run_signing_sdk(
        &self,
        keygen_session_id: &SessionId,
        message_hash: [u8; 32],
    ) -> Result<Vec<u8>> {
        ...
        // Restore keygen session credentials
        ...
        // Restore keygen session to get session info
        let keygen_credentials = keymeld_sdk::SessionCredentials::from_session_secret(&seed)
        ...
        for idx in &self.participants_requiring_approval {
            if *idx < self.sdk_participant_clients.len() {
                ...
                let participant_keygen_credentials =
                    keymeld_sdk::SessionCredentials::from_session_secret(&seed)
                        .map_err(|e| anyhow!("Failed to restore session credentials: {e}"))?;
        ...
        if results.is_empty() {
            return Err(anyhow!("No signature results"));
        }
