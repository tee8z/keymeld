...
quickstart:
    ...
    QUIET=true ./scripts/setup-vsock-sudoers.sh || echo "VSock setup failed, continuing with fallback mode..."
...
test-single-signer:
    ...
    if [ -n "${IN_NIX_SHELL:-}" ]; then
        ...
        ./scripts/start-services.sh
        echo "Running single-signer E2E test..."
        ./scripts/run-demo.sh single-signer
    else
        # Not in nix shell - run everything in a single nix develop session
        nix develop -c bash -c '
            if [ -z "${SKIP_BUILD:-}" ] || [ ! -f target/debug/keymeld-gateway ] || [ ! -f target/debug/keymeld-enclave ] || [ ! -f target/debug/keymeld_demo ]; then
...
mine blocks="6":
    @echo "Mining {{blocks}} regtest blocks..."
...
build-prod:
    @echo "Building production packages with pure Nix..."
...
info:
    #!/usr/bin/env bash
    echo "KeyMeld System Information"
    echo "============================"
    echo ""
    echo "Nix:"
...
# Build an AWS Nitro image (EIF) and its measurement manifest
build-eif:
    @echo "Building AWS Nitro Enclave image and review manifest..."
    nix run .#build-eif

# Production: Deploy reviewed local EIF artifacts with CID discovery
deploy-aws:
